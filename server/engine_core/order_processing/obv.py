...
def _get_available_delivery_partners(addr_state: str, addr_district: str, all_shop_coords: list):
    def _fetch_partners_within_radius(radius_km: float):
        ...
        try:
            for doc in docs:
                ...
                distance = utils.haversine(shop_centroid[0], shop_centroid[1], dp_lat, dp_lon)
                log.info(f"Partner {data.get('dp_name')}({data.get('dp_id')}) radius from centroid: {distance}km")
...
def on_new_obv_order_received_store_pref(data):
    try:
        ...
        try:
            assigned_dp_data = assign_delivery_partner_obv(order_id, store_pref_data, all_shop_coords,
                                                           del_addr_state_as_shop_state,
                                                           del_addr_dist_as_shop_dist)
